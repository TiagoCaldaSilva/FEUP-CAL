...
bool changeMakingBF(unsigned int C[], unsigned int Stock[], unsigned int n, unsigned int T, unsigned int usedCoins[]) {
    unsigned int currentStock[n], result[n], currentCoins = 0, resultCoins = 0, currentIndex = 0, currentValue = 0;
    bool firstTime = true;
    ...
    while(true){
        ...
        currentIndex = 0;
        while(currentStock[currentIndex] == Stock[currentIndex] && currentIndex < n){
            currentIndex++;
        }

        if(n == currentIndex) break;

        for(int i = 0; i < currentIndex; i++) currentStock[i] = 0;

        currentStock[currentIndex]++;
        currentCoins = 0;
        currentValue = 0;
        for(int i = 0; i < n; i++){
            currentCoins += currentStock[i];
            currentValue += currentStock[i] * C[i];
        }

        if(currentValue == T && (firstTime || currentCoins < resultCoins)){
            firstTime = false;
            resultCoins = currentCoins;
            for(int i = 0; i < n; i++){
                result[i] = currentStock[i];
            }
        }
    }

    if(!firstTime){
        for(int i = 0; i < n; i++) usedCoins[i] = result[i];
    }
    return !firstTime;
}
