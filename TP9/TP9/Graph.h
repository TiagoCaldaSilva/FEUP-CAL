...
double Graph<T>::minCostFlow(T source, T sink, double flow) {
    ...
    Vertex<T> * last = findVertex(sink);

    bool first_it = true;
    double total = 0, cost = 0, unit_cost = 0;

    while(total < flow) {
        if (first_it) {
            bellmanFordShortestPath(first);
            first_it = false;
        }
        else dijkstraShortestPath(first);
        if (last->dist == INF) break;
        unit_cost += last->dist;
        ...
        /**
         * Ver o fluxo máximo, podemos não precisar do flow todo
         * O objetivo é alcançar o valor passado em flow
         */
         double temp_flow = std::min(findMinResidualAlongPath(first, last), flow - total);
         total += temp_flow;
         cost += unit_cost * temp_flow;
         augmentFlowAlongPath(first, last, temp_flow);
    }
    return cost;
}
