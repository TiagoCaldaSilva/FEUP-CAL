 ...
 * Variante: size - i:
 *  É um valor inteiro
 *  É positivo, pois i vai ser sempre menor que size
 *  É decrescente, pois i aumenta a cada iteação e size é sempre o mesmo do início ao fim do ciclo
 *
 * Invariante: o valor de r.dmin vai ser sempre o menor valor de dmin dos em relação aos pontos que já foram corridos
 *  É verdadeiro inicialmente, pois ainda não se iterou sobre qualquer ponto
 *  É verdadeira no início e no final de cada iteração visto que a cada iteração se encontramos uma distância menor
 *      do que a já encontrada vamos substituir o valor de r.dmin (e os seus pontos) para o encontrado
 *  No final do ciclo, r vai guardar a distância mínima e os pontos que distam essa distância, logo também é verdade
 *
 */
...
Result nearestPoints_BF(std::vector<Point> &vp) {
    ...
    for(int i = 0; i < vp.size(); i++){
        for(int j = i + 1; j < vp.size(); j++){
            double temp = vp[i].distance(vp[j]);
            if(first || vp[i].distance(vp[j]) < res.dmin){
                first = false;
                res.dmin = temp;
                res.p1 = vp[i];
                res.p2 = vp[j];
...
Result nearestPoints_BF_SortByX(std::vector<Point> &vp) {
    ...
    for(int i = 0; i < vp.size(); i++){
        ...
        for(int j = i + 1; j < vp.size(); j++){
            Point second = vp[j];
            double temp = first.distance(second);

            if(first_time || first.distance(second) < res.dmin){
                first_time = false;
                res.dmin = temp;
                res.p1 = first;
                res.p2 = second;
            }
            else if(second.x - first.x > res.dmin) break;
        }
    }
    return res;
}

Result DC_rec(std::vector<Point> &vp, int start, int end){
    //Only consider 2 elements
    if(end - start == 1) return  {vp[start].distance(vp[end]), vp[start], vp[end]};
    else {
        int middle = (start + end) / 2 + 1;

        Result left = DC_rec(vp, start, middle - 1);
        Result right = DC_rec(vp, middle, end);

        Result r = (left.dmin < right.dmin) ? left:right;

        std::vector<Point> verifyAgain;

        Point middle_left = vp[middle - 1], middle_right = vp[middle];

        for(int i = middle - 1; i >= start; i--){
            if((middle_left.x - vp[i].x) <= r.dmin) verifyAgain.push_back(vp[i]);
            else break;
        }

        for(int i = middle; i <= end; i++){
            if((vp[i].x - middle_right.x) <= r.dmin) verifyAgain.push_back(vp[i]);
            else break;
        }

        for(int i = 0; i < verifyAgain.size(); i++){
            for(int j = i + 1; j < verifyAgain.size(); j++){
                if(verifyAgain[i].distance(verifyAgain[j]) < r.dmin){
                    r = Result(verifyAgain[i].distance(verifyAgain[j]), verifyAgain[i], verifyAgain[j]);
                }
            }
        }
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    sortByX(vp, 0, vp.size()-1);
    //TODO
    return DC_rec(vp, 0, (int)vp.size() - 1);
}

Result DC_rec_MT(std::vector<Point> &vp, int start, int end, int numberT) {
    //Only consider 2 elements
    if (end - start == 1) {
        Point p = vp[end];
        return {vp[start].distance(p), vp[start], p};
    }
    else {
        int middle = (start + end) / 2 + 1;
        ...
        if(numberT > 1) {
            std::thread t([&](){left = DC_rec_MT(vp, start, middle - 1, numberT / 2);});
            right =DC_rec_MT(vp, middle, end, numberT / 2);

            t.join();
        }
        else{
            left = DC_rec_MT(vp, start, middle - 1, 1);
            right = DC_rec_MT(vp, middle, end, 1);
        }
...
Result nearestPoints_DC_MT(std::vector<Point> &vp) {
    ...
    //TODO
    return DC_rec_MT(vp, 0, vp.size() - 1, numThreads);
}
